...
static size_t utf16le2utf8(char *utf8, size_t utf8len, const __u16 *utf16, size_t utf16len)
{
	const __u16 UNI_SUR_HIGH_FIRST =0xD800;
	const __u16 UNI_SUR_HIGH_LAST  =0xDBFF;
	const __u16 UNI_SUR_LOW_FIRST  =0xDC00;
	const __u16 UNI_SUR_LOW_LAST   =0xDFFF;
	const __u32 UNI_MAX_LEGAL_UTF32=0x0010FFFF;
	const __u32 halfBase=0x0010000;
	size_t r=0;
	const __u16 *utf16end=utf16+utf16len;

	while((utf8len>r || !utf8) && utf16!=utf16end){
		__u16 code_unit=blkid_le16(*utf16);

		if(!code_unit || utf16==utf16end){
			if(utf8)*utf8=0;
			break;
		}
		if(code_unit>=UNI_SUR_LOW_FIRST && code_unit<=UNI_SUR_LOW_LAST)return (size_t)-1;
		__u32 ucs4;
		if(code_unit>=UNI_SUR_HIGH_FIRST && code_unit<=UNI_SUR_HIGH_LAST){
			ucs4=code_unit-UNI_SUR_HIGH_FIRST;
			ucs4<<=10;
			++utf16;
			if(utf16==utf16end)return (size_t)-1;
			code_unit=blkid_le16(*utf16);
			if(code_unit<UNI_SUR_LOW_FIRST || code_unit>UNI_SUR_LOW_LAST)return (size_t)-1;
			ucs4+=code_unit-UNI_SUR_LOW_FIRST;
			ucs4+=halfBase;
		}
		else
			ucs4=code_unit;
		...
		if(ucs4<0x80)
			...
			count=3;
		else if(ucs4<=UNI_MAX_LEGAL_UTF32)
			count=4;
		else
			return (size_t)-1;
		if(utf8){
			if(utf8len-r<count)break;
			switch(count){
			case 4: utf8[3] = (char)(0x80 | (ucs4 & 0x3f)); ucs4 = ucs4 >> 6; ucs4 |= 0x10000;
			case 3: utf8[2] = (char)(0x80 | (ucs4 & 0x3f)); ucs4 = ucs4 >> 6; ucs4 |= 0x800;
			case 2: utf8[1] = (char)(0x80 | (ucs4 & 0x3f)); ucs4 = ucs4 >> 6; ucs4 |= 0xc0;
			case 1: utf8[0] = (char)ucs4;
			}
			utf8+=count;
		}
		r+=count;
		      ...
		      unsigned char *buf)
{
	...
	while (1) {
		...
		if (attr_type == MFT_RECORD_ATTR_VOLUME_NAME) {
			const size_t chars_written=utf16le2utf8(label_str,sizeof label_str-1,(const __u16*)(((__u8 *) attr) + val_off),val_len/2);
			if((size_t)chars_written==-1)
				label_str[0] = 0;
			else
				label_str[chars_written] = 0;
		}
	}
